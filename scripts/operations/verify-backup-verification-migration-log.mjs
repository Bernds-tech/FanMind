...
function fail(code) {
  throw new Error(`backup_verification_migration_log_${code}`);
}

export function verifyBackupVerificationMigrationLog(source, notBefore, action) {
  if (
...
export function formatBackupVerificationMigrationDiagnostic(result) {
  const lines = [
    `BACKUP_VERIFICATION_MIGRATION_ACTION=${result.action}`,
    `BACKUP_VERIFICATION_MIGRATION_RESULT=${result.status}`,
  ];
  if (result.errorCode) {
    lines.push(`BACKUP_VERIFICATION_MIGRATION_ERROR=${result.errorCode}`);
  }
  return `${lines.join("\n")}\n`;
}
...
if (direct) {
  ...
  process.stdout.write(
    formatBackupVerificationMigrationDiagnostic(
      verifyBackupVerificationMigrationLog(source, notBefore, action),
    ),
  );
}
