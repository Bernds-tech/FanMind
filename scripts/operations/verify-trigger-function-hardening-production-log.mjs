...
function fail(code) {
  throw new Error(`trigger_function_hardening_production_log_${code}`);
}

export function verifyTriggerFunctionHardeningProductionLog(
  source,
  notBefore,
  action,
) {
  ...
  for (const line of source.split(/\r?\n/u)) {
    ...
    if (
      payload?.level === "info" &&
      payload?.event === "hardening_status" &&
      ...
      payload?.level === "error" &&
      payload?.event === "hardening_failed" &&
...
export function formatTriggerFunctionHardeningProductionDiagnostic(result) {
  const lines = [
    `TRIGGER_FUNCTION_HARDENING_PRODUCTION_ACTION=${result.action}`,
    `TRIGGER_FUNCTION_HARDENING_PRODUCTION_RESULT=${result.status}`,
  ];
  if (result.errorCode) {
    lines.push(
      `TRIGGER_FUNCTION_HARDENING_PRODUCTION_ERROR=${result.errorCode}`,
    );
  }
  return `${lines.join("\n")}\n`;
}
