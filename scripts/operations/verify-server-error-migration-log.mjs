...
function fail(code) {
  throw new Error(`server_error_migration_log_${code}`);
}

export function verifyServerErrorMigrationLog(source, notBefore, action) {
  if (
  ...
  return { action: latest.action, status: latest.status, errorCode: latest.errorCode };
}

export function formatServerErrorMigrationDiagnostic(result) {
  const lines = [
    `SERVER_ERROR_MIGRATION_ACTION=${result.action}`,
    `SERVER_ERROR_MIGRATION_RESULT=${result.status}`,
  ];
  if (result.errorCode) lines.push(`SERVER_ERROR_MIGRATION_ERROR=${result.errorCode}`);
  return `${lines.join("\n")}\n`;
}
...
if (direct) {
  ...
  process.stdout.write(
    formatServerErrorMigrationDiagnostic(
      verifyServerErrorMigrationLog(source, notBefore, action),
    ),
  );
}
