...
2. `project-memory/FANMIND_DEEP_AUDIT_2026-08-19.md`;
...
4. central finishline issue #874 and the exact current Git/PR/CI/runtime/provider state.
...
1. **Project Memory V5 — FM-MEM-005:** implemented on PR #975, exact-head CI/merge still required.
2. **Restore — FM-RST-001, R4:** real isolated DB/Storage/Server-config/Cleanup/Evidence acceptance still open.
3. **Mobile — FM-MOB-001:** signed Android/iOS builds, redirect/EAS/signing, real devices, TestFlight/store/push acceptance open.
4. **AI/Billing — FM-AI-001:** Plus/Ultra product decisions, quality/cost evidence, full current Staging lifecycle and explicit activation open.
5. **Meta/Security — FM-META-001:** external Events Manager/browser evidence, remaining external Meta/App Review E2E and final finishline security evidence open.
6. **Phase 3 — FM-SOC3-001:** Facebook/Instagram/WhatsApp final real E2E not accepted.
7. **Phase 7 — FM-SOC7-001:** TikTok/X/Discord/conditional OnlyFans real connectors not accepted.
8. **Sales handoff — FM-SALES-001:** blocked until required Phase-3/Phase-7 technical acceptance and final Production demo truth.
9. **Legal/Tax/AVV — FM-LEGAL-001:** external approvals remain separate; do not guess.
...
The actual GitHub repository is currently organization-owned as `FanMind/FanMind`. Older restore prose in `docs/SOURCE_OF_TRUTH.md`, `AGENTS.md` and `docs/operations/RESTORE_DRILL.md` still contains pre-transfer `user-owned`/`future-org` wording. Treat that wording as stale. Do **not** infer that the current runner-group policy is still correct solely from ownership; remote admin-policy state must be revalidated before dispatch.
...
1. finish and merge PR #975 only on fully green exact head;
2. continue Restore from current foundation and first unproven gate;
3. Mobile external signing/device/store acceptance;
