...
- Active focus: FanMind Project Memory V6 finishline controls and exact-head acceptance; then continue the real isolated Restore from the first unproven R4 state.
...
- V6 machine truth: `sales_ready=false`, Phase 8 not started; `scripts/fanmind_sales_readiness.py` derives sales readiness and must not be bypassed.
- V6 drift control: `scripts/fanmind_truth_drift_check.py` validates roadmap/source-truth invariants and requires known drift to be recorded, not hidden.
- V6 workflow topology: V6 checks run inside existing `.github/workflows/project-memory-quality.yml`; no extra checkout workflow was retained, preserving the reviewed hosted-workflow inventory.
...
- Exact first unproven governance step: run all checks on the exact latest PR #975 head, including Project Memory Quality V6, and merge only fully green.
- Exact first unproven Restore step after governance: `BACKUP_ACCEPTED -> HOST_REVALIDATED`, beginning with read-only revalidation of host/toolchain/TLS/target and current Organization runner-group/workflow-allowlist/JIT policy.
