...
- Exact current step: complete and merge the repository-only reconciliation. Then wait for `FM-RST-OWNER-003`: a new exact protected authorization limited to rollback-capable extension-baseline provisioning and unchanged full receipt-bound read-only verification. A database-Restore authorization may be considered only after that fingerprint passes.
