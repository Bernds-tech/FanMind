...
- Active Mobile Phase B repository scope: independent Expo/React-Native app with native login, PKCE password recovery, secure session persistence and local purge, dashboard, contact list/search/create/edit/detail, bounded encrypted offline contact overview, contact knowledge, server-side AI reply suggestions, follow-ups, a native wordmark splashscreen and distinct high-resolution iOS/Android app-icon assets. Supabase redirect approval, signed internal builds, real-device verification and store distribution remain separate external release steps.
