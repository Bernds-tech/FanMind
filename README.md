...
- Meta-Nachholqueue: `npm run db:meta-catchup-queue:check` prüft den einzeln freizugebenden SQL-Schritt bytegenau offline. Die manuellen Workflows `FanMind Meta Catch-up Queue Staging Verify` und `FanMind Meta Catch-up Queue Staging Apply` sind an `main`, den exakten geprüften Commit, TLS und ein von Production getrenntes Staging gebunden. Kein Web-Deploy wendet die Migration an oder startet den Worker. Ablauf und Rollback: `docs/operations/META_CATCHUP_QUEUE.md`.
