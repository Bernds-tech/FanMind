...
- Meta-Conversation-Fortsetzung: `npm run db:meta-conversation-continuation:check` prüft die bytegenau gebundene server-only Migration offline. Getrennte exakter-Commit-, TLS- und Staging-gebundene Workflows wenden sie nur nach dem gemeinsamen read-only Rollout-Entscheid an beziehungsweise prüfen Spalten, Paar-/Cursor-Constraint und Browser-Sperren in einer zurückgerollten Transaktion. Der externe Staging-Lauf bleibt offen und aktiviert weder Meta-Abruf noch Analyse oder Worker; Ablauf: `docs/operations/META_CONVERSATION_CONTINUATION_STAGING.md`.
