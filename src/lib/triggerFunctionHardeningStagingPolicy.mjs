...
export const TRIGGER_FUNCTION_HARDENING_VERIFY_CONFIRMATION =
  "verify-trigger-function-hardening";
export const TRIGGER_FUNCTION_HARDENING_APPLY_CONFIRMATION =
...
function evaluateCommitBinding(environment, errors) {
  ...
  const reviewedCommit = clean(
    environment.FANMIND_TRIGGER_FUNCTION_HARDENING_REVIEWED_COMMIT,
  ).toLowerCase();
...
export function evaluateTriggerFunctionHardeningStagingEnvironment(
  environment = {},
  { mode = "verify" } = {},
) {
  if (!MODES.has(mode)) {
