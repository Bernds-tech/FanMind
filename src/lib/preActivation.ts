...
const ASYNC_BILLING_STATUSES = new Set(["pending_sepa_mandate", "past_due", "payment_failed"]);
const PRE_ACTIVATION_BILLING_STATUSES = new Set(["pending_payment_setup", "pending_sepa_mandate", "past_due", "payment_failed", "suspended"]);
...
export function getPreActivationRedirect(workspace: Pick<WorkspaceDashboardRow, "billing_status" | "plan_id" | "name"> | null | undefined): string | null {
  if (!workspace) return "/billing/start";
