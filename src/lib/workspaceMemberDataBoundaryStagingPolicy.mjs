...
export const WORKSPACE_MEMBER_DATA_BOUNDARY_VERIFY_CONFIRMATION =
  "verify-workspace-member-data-boundary";
export const WORKSPACE_MEMBER_DATA_BOUNDARY_APPLY_CONFIRMATION =
...
function evaluateCommitBinding(environment, errors) {
  ...
  const reviewedCommit = clean(
    environment.FANMIND_WORKSPACE_MEMBER_DATA_BOUNDARY_REVIEWED_COMMIT,
  ).toLowerCase();
...
export function evaluateWorkspaceMemberDataBoundaryStagingEnvironment(
  environment = {},
  { mode = "verify" } = {},
) {
  if (!MODES.has(mode)) {
