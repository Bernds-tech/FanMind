...
export function buildWebsiteChatWidgetScript() {
  ...
  root.innerHTML = \`<style>
    ...
    button,textarea,input{font:inherit}.toggle,.send{border:0;border-radius:999px;background:#0787f7;color:#fff;padding:12px 18px;font-weight:700;cursor:pointer;box-shadow:0 10px 30px #001b3d55}
    ...
    h2{font-size:18px;margin:0 0 6px}p{margin:0 0 12px;color:#c8d7e8}.consent{display:flex;gap:8px;align-items:flex-start;margin:10px 0}textarea{box-sizing:border-box;width:100%;min-height:110px;resize:vertical;border:1px solid #477197;border-radius:10px;background:#0b2035;color:#fff;padding:10px}.status{min-height:20px;margin:10px 0 0;color:#a8c9e8}.send{border-radius:10px;margin-top:10px}.send:disabled{opacity:.55;cursor:not-allowed}
  </style><div class="fm"><section class="panel" aria-label="FanMind Nachricht"><h2>Nachricht senden</h2><p>Ihre Nachricht wird an das Team übermittelt. Es erfolgt keine automatische KI-Antwort.</p><textarea maxlength="4000" aria-label="Nachricht" placeholder="Wie können wir helfen?"></textarea><label class="consent"><input type="checkbox"><span>Ich stimme der Verarbeitung meiner Nachricht gemäß Datenschutzhinweis zu.</span></label><button class="send" type="button">Nachricht senden</button><p class="status" role="status" aria-live="polite"></p></section><button class="toggle" type="button" aria-expanded="false">Nachricht</button></div>\`;
