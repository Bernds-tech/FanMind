export const REFERRAL_STAGING_ACCEPTANCE_CONFIRMATION =
  "run-referral-lifecycle-staging-acceptance";
...
export function evaluateReferralStagingAcceptanceEnvironment(
  environment = {},
) {
  const errors = [];
  ...
  const reviewedCommit = clean(
    environment.FANMIND_REFERRAL_STAGING_REVIEWED_COMMIT,
  ).toLowerCase();
  const githubSha = clean(environment.GITHUB_SHA).toLowerCase();
  const primaryWorkspace = clean(
    environment.FANMIND_STAGING_E2E_WORKSPACE_ID,
  ).toLowerCase();
  const secondaryWorkspace = clean(
  ...
  if (
    clean(environment.FANMIND_REFERRAL_STAGING_ACCEPTANCE_CONFIRM) !==
    REFERRAL_STAGING_ACCEPTANCE_CONFIRMATION
  ) {
    errors.push("acceptance_confirmation");
  }
  if (
    !UUID_PATTERN.test(primaryWorkspace) ||
    !UUID_PATTERN.test(secondaryWorkspace) ||
    primaryWorkspace === secondaryWorkspace
  ) {
    errors.push("synthetic_workspaces");
  }
  if (
