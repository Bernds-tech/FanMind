...
export async function POST(request: NextRequest) {
  ...
  if (!data.user) return NextResponse.json({ error: "Bitte melde dich an, um eine Zahlung zu starten." }, { status: 401 });
  ...
  if (!payload.planId || !payload.commercialOption) return NextResponse.json({ error: "Plan oder Zahlungsoption fehlt." }, { status: 400 });

  const config = getStripeConfigStatus();
  if (!config.readyForCheckout) return NextResponse.json({ error: "Zahlung ist noch nicht aktiv konfiguriert. Bitte FanMind kontaktieren." }, { status: 503 });

  const plan = resolveCheckoutPlan(payload.planId, payload.commercialOption);
  if (!plan) return NextResponse.json({ error: "Für diese Paketoption ist kein Checkout verfügbar." }, { status: 400 });
  ...
  if (workspaceResult.workspace.plan_id !== plan.planId || workspaceResult.workspace.commercial_option !== plan.commercialOption) {
    return NextResponse.json({ error: "Die gewählte Zahlungsoption passt nicht zu deinem Workspace." }, { status: 400 });
  }
