-- Disable verify_backup until a non-placeholder server-side verifier is implemented.
-- Idempotent follow-up for environments that already applied 20260711143000.

update public.admin_operation_jobs
set status = 'blocked',
    error_code = 'verify_disabled',
    error_message = 'verify_backup disabled until safe server-side validation ships',
    lease_until = null,
    updated_at = now()
where job_type = 'verify_backup'
  and status in ('queued','claimed','running');

alter table public.admin_operation_jobs drop constraint if exists admin_operation_jobs_job_type_check;
alter table public.admin_operation_jobs add constraint admin_operation_jobs_job_type_check check (job_type in ('health_check','backup_check','backup_snapshot','deployment_check','maintenance_note','manual_review','backup_server_config','backup_database','backup_storage','backup_full'));

alter table public.backup_runs drop constraint if exists backup_runs_backup_type_check;
alter table public.backup_runs add constraint backup_runs_backup_type_check check (backup_type in ('database','storage','configuration','server_config','full'));
...
begin
  ...
  if exists (
    ...
    where job_type in ('backup_server_config','backup_database','backup_storage','backup_full')
