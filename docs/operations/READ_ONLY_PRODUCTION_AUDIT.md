...
- Namen und Aktivierungsstatus der `fanmind-*`-systemd-Units und Timer;
...
- ausschließlich strukturierte Backup-Worker-Ereignisnamen und deren Anzahl.
...
Die Offsite-Konfiguration wird nicht per Shell `source` ausgeführt. Das Skript liest nur drei ausdrücklich erlaubte Konfigurationsfelder als Text:
...
Ein erfolgreicher Audit endet mit:

```text
AUDIT_RESULT=success
```

Für einen belastbaren Operations-Nachweis sollen zusätzlich gelten:
