...
- consent-gesteuerte Meta-Pixel-Infrastruktur als ausdrücklich begrenzte Marketing-Messung auf einer festen Allowlist öffentlicher Seiten: `PageView` sowie `CompleteRegistration` nach erfolgreicher Konto-/Workspace-Erstellung, keine geschützten CRM-/Admin-/Billing-Routen, keine Produkt-Analytics-Suite, kein Laden ohne Einwilligung, keine PII-/CRM-/Billing-Daten, blockierte geschützte same-origin Referrer, kein Advanced Matching und keine Conversions API; ohne gültige `NEXT_PUBLIC_META_PIXEL_ID` vollständig deaktiviert;
...
- Meta Pixel als consent-gesteuerte Marketing-Messung mit `PageView` und einem parameterlosen `CompleteRegistration` erst nach erfolgreicher Konto-/Workspace-Erstellung auf freigegebenen öffentlichen Seiten; geschützte und dynamische CRM-Routen sowie unsichere Query-/Fragmentwerte sind fail-closed ausgeschlossen; weitere Conversion-Events bleiben vorbereitet und unverknüpft, bis sie einzeln freigegeben sind;
